...
export default function OnboardingLanguageScreen({ navigation }) {
  ...
  // Used only on web to force the ScrollView to have a CONCRETE pixel height.
  // RN-web's ScrollView translates to <div overflow:auto>, which only scrolls
  // if its height is bounded. Inheriting `height: 100%` through React
  ...
  return (
    <View style={sty.container}>
      ...
      <ScrollView
        style={[
          sty.scrollFlex,
          // Web: lock the ScrollView's outer div to the window's pixel height.
          // This is the most direct way to guarantee overflow → wheel scroll.
          Platform.OS === "web" && {
            height: winHeight,
            maxHeight: winHeight,
          },
      ...
      >
        ...
        {/* Bottom spacer — must exceed bottom-bar height so the last card
            isn't hidden behind it on small viewports / web. */}
...
const sty = StyleSheet.create({
  container: {
    ...
    // On web, a flex:1 child of an unbounded ancestor has no concrete height,
    // so its descendants can't establish overflow scrolling. Locking the root
    // to viewport height fixes this. (No-op on native — overridden by flex:1.)
    ...(Platform.OS === "web" ? { height: "100vh", overflow: "hidden" } : {}),
  },
  // Web scroll fix:
  //   • flex: 1               → fill the available vertical space.
  //   • minHeight: 0          → CSS flex children default to `min-height: auto`
  //                             which prevents them from shrinking below their
  //                             content size, so overflow:scroll never kicks in.
  //                             Setting minHeight:0 is the standard escape.
  //   • height: '100%' (web)  → some RN-web versions don't apply minHeight:0
  //                             until height is also explicit; belt-and-braces.
  scrollFlex: {
    flex: 1,
    minHeight: 0,
    ...(Platform.OS === "web" ? { height: "100%" } : {}),
  },
  // NOTE: no flexGrow here — flexGrow on contentContainer makes the content
  // fill the parent and there's never any overflow, so the page never scrolls.
