/**
 * Kendra portal API client.
 *
 * Mirrors the mobile app's web transport (frontend/src/services/api.js) and the
 * admin SPA client:
...
// ── Envelope helpers ──────────────────────────────────────────────────────────
export interface ApiMeta { hasMore?: boolean; nextCursor?: string | null; count?: number; unread?: number; total?: number }
...
/** Upload one or more files (multipart/form-data) under a repeated field name. */
export async function apiUploadFiles<T>(url: string, files: File[], fieldName = 'images'): Promise<T> {
