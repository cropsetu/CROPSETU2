...
async function getGeoLib() {
  ...
  try {
    ...
    _lib = null;
    logger.info(
      "[GeoIP] geoip-lite not installed — geo-anomaly detection is inert. Install it to enable offline IP geolocation.",
    );
  }
  return _lib;
}
...
export async function resolveIpGeo(ip) {
  try {
    ...
    const geo =
      g && Array.isArray(g.ll) && g.ll.length === 2
        ? {
            lat: g.ll[0],
            lng: g.ll[1],
    ...
    logger.warn(
      "[GeoIP] resolve failed (treating as unknown): %s",
      err.message,
    );
    return null;
  }
